...
//Get SpookySwap wFTM-USDC LP
async function getFtmUSDCLP() {
	//FTM-USDC Spooky LP (spLP) LP
	var contract = new web3.eth.Contract(abiSpookyLP, ftmUSDCSpookyLP);
	const totalSupplyspLP = await contract.methods.totalSupply().call();
	//console.log("totalSupply FTM-USDC Spooky LP: " + totalSupplyspLP / 1e18);
	const reserves = await contract.methods.getReserves().call();
	USDCReserves = reserves[0];
	//console.log("USDC reserves: " + USDCReserves / 1e6);
	wFTMReserves = reserves[1];
	//console.log("wFTM reserves: " + wFTMReserves / 1e18);
	wFTMUSDC = USDCReserves / wFTMReserves;
	//console.log("wFTM: $" + wFTMUSDC * 1e12);
	return wFTMUSDC;
...
async function readSmartContract() {
  ...
  writeApi.useDefaultTags({host: 'hostVM'})
  
  const wFTMUSDCPrice = await getFtmUSDCLP();
  var wFTMPriceDB = wFTMUSDCPrice * 1e12;

  const point = new Point('wFTM USDC Price').floatField('wFTM-USDC', wFTMPriceDB)
...
setInterval(doStuff, 5000); //time is in ms
